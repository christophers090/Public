...
float AccErrorZ = 0.18;
float GyroErrorX = -1.63;
float GyroErrorY = 3.04;
float GyroErrorZ = 0.85;
...
const int WingRetractionPinL = 7;
const int servo3Pin = 8;
const int WingRetractionPinR = 9;
...
bool retractionEnabledL = true;
bool extensionEnabledL = true;
bool retractionEnabledR = true;
bool extensionEnabledR = true;
...
//Radio communication:
unsigned long channel_1_pwm, channel_2_pwm, channel_3_pwm, channel_4_pwm, channel_5_pwm, channel_6_pwm, channel_7_pwm, channel_8_pwm, channel_9_pwm, channel_10_pwm, channel_11_pwm, channel_12_pwm, channel_13_pwm;
...
float transition_fader = .1; // 1.0 is hover, 0.0 is forward flight
float faderate = 3.0; // seconds to go from max bounds 0 to 1, TUNE
bool transition_fader_enable = false;
// control gains
// hover
float kp_pitch_angle_hover = 0.4;
float kd_pitch_angle_hover = 0.05;
float kp_roll_angle_hover =  0.6;
float kd_roll_angle_hover = 0.05;
float kp_yaw_hover = 0.3;
float kd_yaw_hover = 0.00015;


// forward flight
float kp_pitch_angle_ff = 0.40;
float kd_pitch_angle_ff = 0.05;
float kp_roll_angle_ff = 1.0;
float kd_roll_angle_ff = 0.05;
float kp_yaw_ff = 0.3;
float kd_yaw_ff = 0.00015;

// servo trims
float servo_right_trim = 0.55;
float servo_left_trim = 0.4;
...
void setup() {
  ...
  pinMode(WingRetractionPinL, INPUT_PULLDOWN);
  pinMode(WingRetractionPinR, INPUT_PULLDOWN);
...
void loop() {
  ...
  Ki_yaw = Ki_yaw * transition_fader;
  Ki_roll_angle = Ki_roll_angle * transition_fader;
...
void controlMixer() {
  ...
  m1_command_scaled = (1.0 - transition_fader)*(thro_des - pitch_PID + roll_PID + yaw_PID) + (transition_fader)*(thro_des - pitch_PID + roll_PID + yaw_PID); //Front Left
  m2_command_scaled = (1.0 - transition_fader)*(thro_des - pitch_PID - roll_PID - yaw_PID) + (transition_fader)*(thro_des - pitch_PID - roll_PID - yaw_PID); //Front Right
  m3_command_scaled = (1.0 - transition_fader)*(thro_des + pitch_PID + roll_PID - yaw_PID) + (transition_fader)*(thro_des + pitch_PID - roll_PID + yaw_PID); //Back Right
  m4_command_scaled = (1.0 - transition_fader)*(thro_des + pitch_PID - roll_PID + yaw_PID) + (transition_fader)*(thro_des + pitch_PID + roll_PID - yaw_PID); //Back Left
  ...
  m5_command_scaled = 0;
  m6_command_scaled = 0;
  s2_command_scaled = pitch_PID + 4.5;
  //s4_command_scaled = 0;
  s5_command_scaled = 0;
  s6_command_scaled = 0;
  s7_command_scaled = 0;
...
void Madgwick6DOF(float gx, float gy, float gz, float ax, float ay, float az, float invSampleFreq) {
  ...
  */
  ...
  roll_IMU = (transition_fader)*(atan2(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2)*57.29577951) + (1.0 - transition_fader)*(asin(constrain(2.0f * (q0*q1 + q2*q3),-0.99999,0.99999))*57.29577951); // (1.0 - transition_fader)*(ff) + (transition_fader)*(hover)
  pitch_IMU = (transition_fader)*(-asin(constrain(-2.0f * (q1*q3 - q0*q2),-0.99999,0.99999))*57.29577951) + (1.0 - transition_fader)*(atan2(q1*q3 - q0*q2, 0.5f - (q1*q1 + q2*q2))*57.29577951-90.0f);
  yaw_IMU = (transition_fader)*(-atan2(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3)*57.29577951) + (1.0 - transition_fader)*(-atan2(-q1*q2 + q0*q3, 0.5f - (q1*q1 + q3*q3))*57.29577951);
  float temp = GyroZ;
  GyroZ = (transition_fader)*(GyroZ) + (1.0 - transition_fader)*(GyroX);
  GyroX = (transition_fader)*(GyroX) + (1.0 - transition_fader)*(-temp);
}
...
void scaleCommands() {
  ...
  s2_command_PWM = ((-s2_command_scaled*12) + 90);
...
void comandServos() {
  ...
  bool limitSWL = digitalRead(WingRetractionPinL);

  if(channel_9_pwm > 1700 && retractionEnabledL) {
    s1_command_PWM = 0;

    if(limitSWL && extensionEnabledL){
      retractionEnabledL = false;
    }

  } else if (channel_10_pwm > 1700 && extensionEnabledL){
    s1_command_PWM = 180; 

    if(limitSWL && retractionEnabledL){
      extensionEnabledL = false;
    }
  ...
  if(!limitSWL){
    extensionEnabledL = true;
    retractionEnabledL = true;
  }

  bool limitSWR = digitalRead(WingRetractionPinR);

  if(channel_9_pwm > 1700 && retractionEnabledR) {
    s3_command_PWM = 180;

    if(limitSWR && retractionEnabledR){
      retractionEnabledR = false;
    }

  } else if (channel_10_pwm > 1700 && retractionEnabledR){
    s3_command_PWM = 0; 
...
void onReceiveRcChannels(serialReceiverLayer::rcChannels_t *rcChannels) {
  channel_1_pwm = crsf->rcToUs(crsf->getChannel(3));
  channel_2_pwm = crsf->rcToUs(crsf->getChannel(4));
  channel_3_pwm = crsf->rcToUs(crsf->getChannel(2));
  channel_4_pwm = crsf->rcToUs(crsf->getChannel(1));
  ...
  channel_9_pwm = crsf->rcToUs(crsf->getChannel(9));
  channel_10_pwm = crsf->rcToUs(crsf->getChannel(10));
  channel_11_pwm = crsf->rcToUs(crsf->getChannel(11));
  channel_12_pwm = crsf->rcToUs(crsf->getChannel(12));
  channel_13_pwm = crsf->rcToUs(crsf->getChannel(13));

  channel_4_pwm = 3000 - channel_4_pwm;
...
void printMotorCommands() {
  if (current_time - print_counter > 10000) {
    ...
    Serial.print(F(" transition_fader: "));
...
void getFadeFactor() {
  ...
  if(channel_6_pwm < 1700){
    transition_fader = 1;
    transition_fader_enable = false;
  } else if(channel_6_pwm > 1700 && !transition_fader_enable) {
    if(channel_11_pwm < 1010){
      transition_fader_enable = true;
    }
  } else if(channel_6_pwm > 1700 && transition_fader_enable) {
    transition_fader = (2000 - channel_11_pwm)/1000.0;
  }
